...
final class AXElementRegistry {
    ...
    func lock() {
        isLocked = true
        logger.debug("Registry locked for user interaction")
    }

    /// Unlocks the registry after user interaction
    func unlock() {
        isLocked = false
        logger.debug("Registry unlocked")
    }
    ...
    func lookup(_ id: AXElementID) -> AXUIElement? {
        logger.debug("lookup() is deprecated - elements should be acquired fresh at injection time")
        return nil
    }
    ...
    func getCurrentElement() -> AXUIElement? {
        logger.debug("getCurrentElement() is deprecated - elements should be acquired fresh at injection time")
        return nil
    }
}
