...
actor GoogleAnalyticsTracker: AnalyticsTracker {
    ...
    func initialize() async {
        logger.debug("Google Analytics tracker initialized")
        logger.debug("Measurement ID: \(self.measurementId)")
        logger.debug("Client ID: \(self.clientId)")
    ...
    func reset() async {
        ...
        userProperties = nil
        logger.debug("Google Analytics tracker reset")
    }
    ...
    func setEnabled(_ enabled: Bool) async {
        _isEnabled = enabled
        logger.debug("Google Analytics tracker \(enabled ? "enabled" : "disabled")")
    }
    ...
    private func sendEvents(_ events: [QueuedEvent]) async {
        ...
        guard let url = urlComponents.url else {
            logger.debug("Failed to build GA4 URL")
            return
        }
        ...
        do {
            ...
            if let httpResponse = response as? HTTPURLResponse {
                if httpResponse.statusCode == 204 {
                    ...
                    logger.debug("GA4 returned status code: \(httpResponse.statusCode)")
                }
            }
        } catch {
            logger.debug("Failed to send events to GA4: \(error.localizedDescription)")
