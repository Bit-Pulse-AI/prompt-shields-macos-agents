...
actor ConsoleAnalyticsTracker: AnalyticsTracker {
    ...
    func initialize() async {
        logger.debug("📊 Console Analytics Tracker initialized")
    }
    ...
    func track(_ event: AnalyticsEvent) async {
        ...
        logger.debug("📊 [\(event.category)] \(event.name) - {\(params)}")
    }
    ...
    func setUserProperties(_ properties: AnalyticsUserProperties) async {
        ...
        logger.debug("📊 User properties set: {\(propsString)}")
    }

    func reset() async {
        userProperties = nil
        logger.debug("📊 Console Analytics Tracker reset")
    }
    ...
    func setEnabled(_ enabled: Bool) async {
        _isEnabled = enabled
        logger.debug("📊 Console tracker \(enabled ? "enabled" : "disabled")")
    }
}
