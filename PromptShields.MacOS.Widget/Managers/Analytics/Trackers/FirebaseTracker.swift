...
actor FirebaseTracker: AnalyticsTracker {
    ...
    func initialize() async {
        logger.debug("Firebase tracker initialized")
        logger.debug("Project ID: \(self.projectId)")
        logger.debug("Installation ID: \(self.installationId)")
    ...
    func reset() async {
        ...
        userProperties = nil
        logger.debug("Firebase tracker reset")
    }
    ...
    func setEnabled(_ enabled: Bool) async {
        _isEnabled = enabled
        logger.debug("Firebase tracker \(enabled ? "enabled" : "disabled")")
    }
    ...
    func endTrace(name: String, duration: TimeInterval? = nil, error: String? = nil) {
        guard var trace = activeTraces.removeValue(forKey: name) else {
            logger.debug("Trace not found: \(name)")
            return
        }
    ...
    private func sendPerformancePayload(_ payload: PerformancePayload) async {
        ...
        guard let url = URL(string: urlString) else {
            logger.debug("Invalid Firebase Performance URL")
            return
        }
        ...
        do {
            ...
            if let httpResponse = response as? HTTPURLResponse {
                if httpResponse.statusCode == 200 || httpResponse.statusCode == 204 {
                    ...
                    logger.debug("Firebase Performance returned status: \(httpResponse.statusCode)")
                }
            }
        } catch {
            logger.debug("Failed to send performance data: \(error.localizedDescription)")
        }
    }
    ...
    private func sendCrashlyticsPayload(_ payload: CrashlyticsPayload, isFatal: Bool) async {
        ...
        guard let url = URL(string: urlString) else {
            logger.debug("Invalid Crashlytics URL")
            return
        }
        ...
        do {
            ...
            if let httpResponse = response as? HTTPURLResponse {
                if httpResponse.statusCode == 200 || httpResponse.statusCode == 204 {
                    logger.debug("\(isFatal ? "Crash" : "Error") report sent successfully")
                } else {
                    logger.debug("Crashlytics returned status: \(httpResponse.statusCode)")
                }
            }
        } catch {
            logger.debug("Failed to send \(isFatal ? "crash" : "error") report: \(error.localizedDescription)")
        }
    }
}
