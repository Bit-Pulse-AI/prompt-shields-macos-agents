...
actor PostHogTracker: AnalyticsTracker {
    ...
    func initialize() async {
        logger.debug("PostHog tracker initialized")
        logger.debug("Host: \(self.host)")
        logger.debug("Distinct ID: \(self.distinctId)")
    ...
    func reset() async {
        ...
        logger.debug("PostHog tracker reset with new distinct ID")
    }
    ...
    func setEnabled(_ enabled: Bool) async {
        _isEnabled = enabled
        logger.debug("PostHog tracker \(enabled ? "enabled" : "disabled")")
    }
        ...
        do {
            ...
            if let httpResponse = response as? HTTPURLResponse {
                if httpResponse.statusCode == 200 {
                    ...
                    logger.debug("PostHog identify returned status: \(httpResponse.statusCode)")
                }
            }
        } catch {
            logger.debug("Failed to send PostHog identify: \(error.localizedDescription)")
        }
    }
        ...
        do {
            ...
            if let httpResponse = response as? HTTPURLResponse {
                if httpResponse.statusCode == 200 {
                    ...
                    logger.debug("PostHog returned status code: \(httpResponse.statusCode)")
                }
            }
        } catch {
            logger.debug("Failed to send events to PostHog: \(error.localizedDescription)")
