...
final class AnalyticsManager: ObservableObject {
    ...
    func initialize() async {
        guard !isInitialized else {
            logger.debug("AnalyticsManager already initialized")
            return
        }

        logger.debug("Initializing AnalyticsManager...")
        ...
        isInitialized = true
        logger.debug("AnalyticsManager initialized with \(self.trackers.count) tracker(s)")
    ...
    func addTracker(_ tracker: any AnalyticsTracker) async {
        ...
        if trackers.contains(where: { $0.identifier == tracker.identifier }) {
            logger.debug("Tracker \(tracker.identifier) already registered")
            return
        }
        ...
        logger.debug("Added tracker: \(tracker.identifier)")
    }
    ...
    func removeTracker(identifier: String) async {
        ...
        activeTrackerCount = trackers.count
        logger.debug("Removed tracker: \(identifier)")
    }
    ...
    func setUserProperties(_ properties: AnalyticsUserProperties) async {
        ...
        logger.debug("User properties updated across all trackers")
    }
    ...
    func reset() async {
        ...
        logger.debug("All trackers reset")
    }
